...
/* set temp */
uint8_t MeCom_SetTemp(uint8_t Address, float temp) {
	uint8_t result = 0;
	...
	start.Min = 0;
	start.Max = 3;
	result = MeCom_ParValuel(Address, 2010, 1, &start, MeSet); //
	...
	/* set temp inside ram */
	result = MeCom_ParValuef(Address, 50012, 1, &temperature, MeSet); //1 set
	if (result == 0) {
		printf("MeCom error\n");
		return result;
	}
	...
	start.Min = 0;
	start.Max = 1;
	result = MeCom_ParValuel(Address, 50011, 1, &start, MeSet); //1 set
	...
	start.Min = 0;
	start.Max = 3;
	result = MeCom_ParValuel(Address, 2010, 1, &start, MeSet); //1 set
